...
#	&log_request_attributes;
	...
	my $nas      = $RAD_REQUEST{'NAS-IP-Address'};

  	my $cmd = "/usr/local/bin/fb_smartradius/src/lab_auth.py $nas $username \"$password\"";
        my $out = `$cmd`;
	print "OUT = $out\n";
        if ($out =~ /OK/){
		$RAD_CHECK{'Auth-Type'} = "Accept";
		$out =~ s/ +//g;
		if ($out =~ /OK:(cisco-avpair=)(.+)/i){
	          $RAD_REPLY{'Cisco-AVPair'} = $2;
		}
                if ($out =~ /OK:(arista-avpair=)(.+)/i){
                  $RAD_REPLY{'Arista-AVPair'} = $2;
		  $RAD_REPLY{'Service-Type'} = "NAS-Prompt-User";
                }
